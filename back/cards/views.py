...
def generate_card(request):
    if request.method == "POST":
        ...
        text = request.POST.get("text", "Happy Journey!")  # 카드에 추가할 텍스트
        font_file = request.FILES.get("font")  # 업로드된 폰트 파일

        if not font_file:
            return JsonResponse({"error": "No font file provided"}, status=400)
        ...
        # 폰트 처리
        try:
            font_path = f"/tmp/{font_file.name}"  # 임시 폰트 파일 저장 경로
            with open(font_path, "wb") as f:
                f.write(font_file.read())
            font = ImageFont.truetype(font_path, 50)  # 업로드된 폰트 로드
        except Exception as e:
            return JsonResponse({"error": f"Failed to load font: {str(e)}"}, status=500)

        # 카드 디자인
        try:
            card = Image.new("RGB", (800, 600), "white")  # 흰색 배경 카드 생성
            card.paste(background_image.resize((800, 400)), (0, 0))  # 배경 이미지 삽입

            # 텍스트 추가
            draw = ImageDraw.Draw(card)
            draw.text((50, 500), text, font=font, fill="black")  # 텍스트 삽입
        except Exception as e:
            return JsonResponse({"error": f"Failed to design card: {str(e)}"}, status=500)
